...
// Find the index of "displayName: 'Strings'" or "displayName: 'Numbers'"
let startIdx = -1;
lines.forEach((line, idx) => {
  if (line.includes("displayName: 'Strings'") || line.includes("name: 'rules'")) {
    if (startIdx === -1) startIdx = idx;
  }
});

console.log('=== SwitchV1 RULES SCHEMA DETAILS ===');
if (startIdx !== -1) {
  for (let i = startIdx; i < startIdx + 120 && i < lines.length; i++) {
    console.log(`${i + 1}: ${lines[i]}`);
  }
} else {
  console.log('Rules schema not found');
}
